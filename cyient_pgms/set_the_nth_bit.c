 ...
 * take user input
 * take the pin to set
 * set the desired pin in the user input using or operator
 *
 *
 *
...
int set_bit(int num1,int pin_no)		//function to set the bit
{
	num1 |= (1<<pin_no);	//set the nth bit using bitwise and or and shift operator
...
int main()
{
	...
	print_binary(num1);
	printf("\n output in integaer :%d",set_bit(num1,pin_no));	//call the function to return the output in integaer format
	return 0;
}
