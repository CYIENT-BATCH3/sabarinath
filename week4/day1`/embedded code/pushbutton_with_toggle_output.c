...
#include <xc.h>
#define __XTAL_FREQ 20000000                //defining external crystal oscillator frequancy

void main(void)
{
    int val =0;
    TRISBbits.RB0 = 1;                      //RB0 as input
    TRISCbits.RC0 = 0;                      //RC0 as output
    INTCON2bits.RBPU = 0;                   //turn on pullup resistors
    PORTCbits.RC0 = 0;                      //intilise rc0 as 0
    
    while(1)                                //infinite loop
    {
        if(PORTBbits.RB0 == 0)              //if switch is pressed
        {
            val = !val;                     //negate the value
            PORTCbits.RC0 = val;            //assighn the avl to rc0
           for(int long i =0;i<50000;i++);  //delay for the debouncing time
        }
