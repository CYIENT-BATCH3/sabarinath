...
int main()
{   char val = 'a';				// declaring a value for transmitting and assigning it with character a
    ...
    //RCSTAbits.SPEN= 1;
    while(1)					//start the infinte loop
    {
    while(TXIF ==0);				//check for TXIF to become 1 
    TXREG = val;				// giving the character to TXREG buffer register for transmitting data
    }
    return 0;
}
